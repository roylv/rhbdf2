...
pdf("~/public_html/rhbdf/sampMrg8.pdf")
...
#	plot(main=Xxx[j], cex.main=cexMain, mrg$bp[int][ind]/1e6, mrg$logP.merge[int][ind], ylim=c(0, 1.2+max( mrg$logP.merge[int][ind])), lwd=lwd, col=colMrg, pch=pch, xlab="Mb", ylab=ylab, xlim=c((mrg$bp[int][ind]/1e6)[1]+strt,tail(mrg$bp[int][ind]/1e6, n=1)-endd))
	plot(main=Xxx[j], cex.main=cexMain, mrg2$bp/1e6, mrg2$logP.merge, ylim=c(0, 1.2+max( mrg2$logP.merge)), lwd=lwd, col=colMrg, pch=pch, xlab="Mb", ylab=ylab) #xlim=c((mrg$bp[int][ind]/1e6)[1]+strt,tail(mrg$bp[int][ind]/1e6, n=1)-endd))
